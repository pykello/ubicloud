...
class Prog::Vm::Nexus < Prog::Base
  ...
  def self.assemble(public_key, project_id, name: nil, size: DEFAULT_SIZE,
    ...
    if boot_image.include?("@")
      ...
      machine_image_version = lookup_machine_image_version(project_id, location_id, image_name, image_version, storage_volumes.first[:size_gib])
      vm_storage_volumes[boot_disk_index][:machine_image_version_id] = machine_image_version.id
    end
