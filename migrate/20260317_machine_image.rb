...
Sequel.migration do
  change do
    create_table(:machine_image) do
      ...
      constraint(:arch_is_valid) { Sequel.lit("arch IN ('x64', 'arm64')") }
    end
    ...
    create_table(:machine_image_store) do
      ...
      # specifies the default store, and can be overridden for each project.
      foreign_key :project_id, :project, type: :uuid, null: false
      foreign_key :location_id, :location, type: :uuid, null: false
    ...
    create_table(:machine_image_version_metal) do
      ...
      constraint(:size_set_if_enabled) do
        Sequel.lit("NOT enabled OR archive_size_mib IS NOT NULL")
      end
    end
