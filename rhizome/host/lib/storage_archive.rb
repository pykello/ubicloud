...
class StorageArchive
  ...
  def build_target_config
    ...
    parts = [Toml.section("target", target)]

    parts << Toml.section("secrets.s3-key-id", {
      "source.inline" => @target_conf["access_key_id"]
    })
    parts << Toml.section("secrets.s3-secret-key", {
      "source.inline" => @target_conf["secret_access_key"]
    })
    parts << Toml.section("secrets.archive-kek", {
      "source.inline" => @target_conf["archive_kek"]["key"],
      "encoding" => "base64"
    })

    if @target_conf["session_token"]
      parts << Toml.section("secrets.s3-session-token", {
    ...
    # acceptable.
    parts << Toml.section("danger_zone", {
      "enabled" => true,
