...
class StorageVolume
  ...
  def vhost_backend_start(key_wrapping_secrets)
    ...
    KekPipe.with_kek_pipe(sp.kek_pipe, owner: @vm_name) do |kek_pipe|
      r "systemctl", "start", vhost_user_block_service
      KekPipe.write_kek_to_pipe(kek_pipe, vhost_backend_kek(key_wrapping_secrets))
    end
  end
