...
RSpec.describe Clover, "machine-image" do
  ...
  describe "authenticated" do
    ...
    describe "delete version" do
      ...
      # The status flip + latest_version_id reassignment + R2 wipe now
      # happen asynchronously inside VersionMetalNexus; the route just
      # queues the destroy semaphore.

      it "queues destroy for a ready non-latest version" do
        mi_version_metal
        ...
        within("#miv-#{other_version.ubid}") { click_button(class: "delete-btn") }
        expect(page).to have_flash_notice("Version '#{other_version.version}' is being deleted")
        expect(other_metal.reload.destroy_set?).to be true
      end

      it "queues destroy for the latest version" do
        mi_version_metal
        mi.update(latest_version_id: mi_version.id)
        visit "#{project.path}/location/#{TEST_LOCATION}/machine-image/#{mi.name}/versions"
        ...
        expect(mi_version_metal.reload.destroy_set?).to be true
      end

      it "queues destroy on a creating version (the archive label sees it next iteration)" do
        mi_version_metal
        visit "#{project.path}/location/#{TEST_LOCATION}/machine-image/#{mi.name}/versions"
