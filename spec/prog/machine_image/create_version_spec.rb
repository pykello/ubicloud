...
RSpec.describe Prog::MachineImage::CreateVersion do
  ...
  describe ".assemble" do
    ...
    it "fails when no store exists for the location" do
      store.destroy
      expect {
        described_class.assemble(machine_image, "1.0", source_vm)
      }.to raise_error(/no machine image store found/)
    end
    ...
    it "creates a machine image version and strand" do
      ...
      expect(metal.store_prefix).to eq("#{project.ubid}/#{machine_image.ubid}/1.0")
  ...
  describe "#finish" do
    ...
    it "enables machine image version and updates latest version" do
      ...
      expect(mi_version.metal.archive_size_mib).to eq(10)
  ...
  describe "#archive_size_bytes" do
    it "sums object sizes across pages" do
      ...
      allow(s3).to receive(:list_objects_v2).with(bucket: "test-bucket", prefix: metal.store_prefix).and_return([page1, page2])
