...
RSpec.describe Prog::MachineImage::CreateVersionMetalFromUrl do
  ...
  describe ".assemble" do
    before { vhost_block_backend }

    it "fails when no vm host with archive support exists in location" do
      VhostBlockBackend.dataset.destroy

      expect {
        described_class.assemble(mi_version, url, sha256sum, store)
    ...
    it "creates a machine image version metal and strand" do
      ...
      strand = described_class.assemble(new_mi_version, url, sha256sum, store)
      ...
      expect(strand.stack.first["vhost_block_backend_version"]).to eq(vhost_block_backend.version)
  ...
  describe "#archive" do
    ...
    before do
      allow(prog).to receive_messages(vm_host:, archive_params_json: "{\"target_conf\":{}}")
    end
    ...
    it "reads stats, cleans daemon and hops to finish when daemon succeeded" do
      ...
      expect(sshable).to receive(:cmd).with("cat :stats_path", stats_path:).and_return('{"physical_size_bytes": 10485760, "logical_size_bytes": 1073741824}')
    ...
    it "starts daemon when status is NotStarted" do
      ...
      expect(sshable).to receive(:d_run).with(daemon_name,
        "sudo", "host/bin/archive-url", url, sha256sum, vhost_block_backend.version, stats_path,
        stdin: "{\"target_conf\":{}}", log: false)
  ...
  describe "#finish" do
    before {
      refresh_frame(prog, new_values: {"archive_size_bytes" => 10 * 1024 * 1024, "logical_size_bytes" => 20 * 1024 * 1024})
  ...
  describe "#archive_params_json" do
    it "generates JSON payload with store credentials and no kek" do
