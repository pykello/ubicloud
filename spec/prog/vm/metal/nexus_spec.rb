...
RSpec.describe Prog::Vm::Metal::Nexus do
  ...
  describe "#wait_storage_catchup" do
    ...
    it "naps if a volume is not caught up" do
      ...
      allow(vm.vm_host.sshable).to receive(:_cmd).with("echo #{payload.to_json.shellescape} | sudo nc -U /var/storage/#{vm.inhost_name}/0/rpc.sock -w 1 | head -n 1").and_return('{"status": {"stripes": {"fetched": 50, "source": 100}}}')
    ...
    it "clears machine_image_version_id and hops to wait when volume is caught up" do
      ...
      payload = {command: "status"}
      allow(vm.vm_host.sshable).to receive(:_cmd).with("echo #{payload.to_json.shellescape} | sudo nc -U /var/storage/#{vm.inhost_name}/1/rpc.sock -w 1 | head -n 1").and_return('{"status": {"stripes": {"fetched": 100, "source": 100}}}')
      expect { nx.wait_storage_catchup }.to hop("wait")
