...
RSpec.describe Prog::Test::HetznerServer do
  ...
  before {
    ...
    allow(hs_test).to receive_messages(hetzner_api:, vm_host:)
  }
  ...
  describe "#run_integration_specs" do
    it "hops to wait" do
      ...
      expect(vm_host.sshable).to receive(:_cmd).with("sudo mkdir -p #{tmp_dir}")
      expect(vm_host.sshable).to receive(:_cmd).with("sudo chmod a+rw #{tmp_dir}")
      expect(vm_host.sshable).to receive(:_cmd).with(
        "sudo RUN_E2E_TESTS=1 bundle exec rspec host/e2e"
      )
      expect(vm_host.sshable).to receive(:_cmd).with("sudo rm -rf #{tmp_dir}")
  ...
  describe "#verify_vm_dir_purged" do
    it "doesn't fail if /vm is empty" do
      expect(vm_host.sshable).to receive(:_cmd).with("sudo ls -1 /vm").and_return("")
      expect { hs_test.verify_vm_dir_purged }.to hop("verify_storage_files_purged")
    end

    it "fails if /vm is not empty" do
      expect(vm_host.sshable).to receive(:_cmd).with("sudo ls -1 /vm").and_return("vm1\nvm2\n")
  ...
  describe "#verify_storage_files_purged" do
    it "succeeds if /var/storage and /var/storage/vhost are empty" do
      expect(vm_host.sshable).to receive(:_cmd).with("sudo ls -1 /var/storage").and_return("vhost\nimages\n")
      expect(vm_host.sshable).to receive(:_cmd).with("sudo ls -1 /var/storage/vhost").and_return("")
    ...
    it "fails if /var/storage/vhost has files" do
      expect(vm_host.sshable).to receive(:_cmd).with("sudo ls -1 /var/storage").and_return("vhost\nimages\n")
      expect(vm_host.sshable).to receive(:_cmd).with("sudo ls -1 /var/storage/vhost").and_return("file1\nfile2\n")
      expect { hs_test.verify_storage_files_purged }.to hop("failed")
  ...
  describe "#wait_vm_host_destroyed" do
    ...
    it "hops to finish if the vm host destroyed" do
      allow(hs_test).to receive(:vm_host).and_call_original
      refresh_frame(hs_test, new_values: {"vm_host_id" => SecureRandom.uuid})
  ...
  describe "#vm_host" do
    it "returns the vm_host" do
      vmh = Prog::Vm::HostNexus.assemble("2.2.2.2").subject
