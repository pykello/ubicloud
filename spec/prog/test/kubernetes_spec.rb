...
RSpec.describe Prog::Test::Kubernetes do
  ...
  describe "#test_node_not_deleted_during_copy" do
    before do
      nodepool = kubernetes_cluster.nodepools.first
      nodepool.update(node_count: 2)
      Prog::Kubernetes::KubernetesNodeNexus.assemble(kubernetes_service_project_id, sshable_unix_user: "ubi", name: "cp-node", location_id: Location::HETZNER_FSN1_ID, size: "standard-2", storage_volumes: [{size_gib: 40}], boot_image: "kubernetes-v1.33", private_subnet_id: private_subnet.id, enable_ip4: true, kubernetes_cluster_id: kubernetes_cluster.id, kubernetes_nodepool_id: nil)
      Prog::Kubernetes::KubernetesNodeNexus.assemble(kubernetes_service_project_id, sshable_unix_user: "ubi", name: "w1-node", location_id: Location::HETZNER_FSN1_ID, size: "standard-2", storage_volumes: [{size_gib: 40}], boot_image: "kubernetes-v1.33", private_subnet_id: private_subnet.id, enable_ip4: true, kubernetes_cluster_id: kubernetes_cluster.id, kubernetes_nodepool_id: nodepool.id)
      Prog::Kubernetes::KubernetesNodeNexus.assemble(kubernetes_service_project_id, sshable_unix_user: "ubi", name: "w2-node", location_id: Location::HETZNER_FSN1_ID, size: "standard-2", storage_volumes: [{size_gib: 40}], boot_image: "kubernetes-v1.33", private_subnet_id: private_subnet.id, enable_ip4: true, kubernetes_cluster_id: kubernetes_cluster.id, kubernetes_nodepool_id: nodepool.id)
      expect(kubernetes_test).to receive(:kubernetes_cluster).and_return(kubernetes_cluster).at_least(:once)
    end
