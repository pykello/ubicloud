...
RSpec.describe Prog::Test::Vm do
  ...
  describe "#verify_systemd_unit_status_after_shutdown" do
    ...
    it "verifies systemd unit status and hops" do
      expect(vm_test.vm.vm_host.sshable).to receive(:_cmd).with("systemctl is-active vm123456").and_return("inactive\n")
    ...
    it "fails if systemd unit status is unexpected" do
      ...
      expect(vm_test.strand).to receive(:update).with(exitval: {msg: "VM should be inactive after shutdown command, but is unknown"})
