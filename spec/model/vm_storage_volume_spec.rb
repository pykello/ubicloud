...
RSpec.describe VmStorageVolume do
  ...
  describe "#rpc" do
    it "sends a json payload to the rpc socket and parses the response" do
      ...
      allow(vol.vm.vm_host.sshable).to receive(:_cmd).with("echo #{payload.to_json.shellescape} | sudo nc -U /var/storage/#{vm.inhost_name}/1/rpc.sock -w 1 | head -n 1").and_return('{"version":"v0.4.1"}')
  ...
  describe "#caught_up?" do
    ...
    it "is true when stripes fetched equals source" do
      ...
      allow(vol.vm.vm_host.sshable).to receive(:_cmd).with("echo #{payload.to_json.shellescape} | sudo nc -U /var/storage/#{vm.inhost_name}/1/rpc.sock -w 1 | head -n 1").and_return('{"status": {"stripes": {"fetched": 100, "source": 100}}}')
      expect(vol.caught_up?).to be true
    end

    it "is false when stripes fetched differs from source" do
      payload = {"command" => "status"}
      allow(vol.vm.vm_host.sshable).to receive(:_cmd).with("echo #{payload.to_json.shellescape} | sudo nc -U /var/storage/#{vm.inhost_name}/1/rpc.sock -w 1 | head -n 1").and_return('{"status": {"stripes": {"fetched": 50, "source": 100}}}')
