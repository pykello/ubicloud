...
RSpec.describe Al do
  ...
  describe "update" do
    let(:vol) {
      [{"size_gib" => 5, "use_bdev_ubi" => false, "boot" => false}]
    }
    ...
    it "updates resources" do
      ...
      described_class.allocate(vm, [{"size_gib" => 85, "use_bdev_ubi" => false, "boot" => false},
        {"size_gib" => 95, "use_bdev_ubi" => false, "boot" => false}])
      vmh.reload
    ...
    it "updates pci devices" do
      ...
      available_storage = vmh.storage_devices.sum { it.available_storage_gib }
      described_class.allocate(vm, [{"size_gib" => 85, "use_bdev_ubi" => false, "boot" => false},
        {"size_gib" => 95, "use_bdev_ubi" => false, "boot" => false}], gpu_count: 1)
      vmh.reload
      expect(vm.vm_storage_volumes.detect { it.disk_index == 0 }.size_gib).to eq(85)
      expect(vm.vm_storage_volumes.detect { it.disk_index == 1 }.size_gib).to eq(95)
      expect(StorageDevice[vm.vm_storage_volumes.detect { it.disk_index == 0 }.storage_device_id].name).to eq("stor2")
    ...
    it "prefers gpus on single numa node" do
      ...
      described_class.allocate(vm, [{"size_gib" => 85, "use_bdev_ubi" => false, "boot" => false},
        {"size_gib" => 95, "use_bdev_ubi" => false, "boot" => false}], gpu_count: 2)
      vmh.reload
    ...
    it "can allocate gpus across numa nodes" do
      ...
      PciDevice.create(vm_host_id: vmh.id, slot: "05:00.0", device_class: "0302", vendor: "vd", device: "27b0", numa_node: nil, iommu_group: 5)
      described_class.allocate(vm, [{"size_gib" => 85, "use_bdev_ubi" => false, "boot" => false},
        {"size_gib" => 95, "use_bdev_ubi" => false, "boot" => false}], gpu_count: 5)
      vmh.reload
    ...
    it "allocates a GPU partition" do
      ...
      described_class.allocate(vm, [{"size_gib" => 95, "use_bdev_ubi" => false, "boot" => false}], gpu_count: 4)
      vmh.reload
    ...
    it "allocates to non-overlapping GPU partition" do
      ...
      vol = [{"size_gib" => 95, "use_bdev_ubi" => false, "boot" => false}]
    ...
    it "succeeds allocation when track_written is set and host has vhost block backend v0.4.0+" do
      ...
      vm = create_vm
      vol = [{"size_gib" => 5, "use_bdev_ubi" => false, "boot" => false, "track_written" => true, "vring_workers" => 1}]
      described_class.allocate(vm, vol)
      expect(vm.reload.vm_host_id).to eq(vmh.id)
      expect(vm.vm_storage_volumes.first.track_written).to be(true)
    end
    ...
    it "fails allocation when track_written is set but no host has vhost block backend v0.4.0+" do
      vm = create_vm
      vol = [{"size_gib" => 5, "use_bdev_ubi" => false, "boot" => false, "track_written" => true}]
    ...
    it "creates volume with encryption key if storage is not read-only" do
      vm = create_vm
      described_class.allocate(vm, [{"size_gib" => 5, "use_bdev_ubi" => false, "read_only" => false, "boot" => false}])
    ...
    it "uses vhost block backend if available" do
      ...
      vm = create_vm
      described_class.allocate(vm, [{"size_gib" => 5, "use_bdev_ubi" => false, "boot" => false, "vring_workers" => 3}])
    ...
    it "allocates the latest active boot image for boot volumes" do
      ...
      vm = create_vm
      described_class.allocate(vm, [{"size_gib" => 5, "use_bdev_ubi" => false, "boot" => true}])
    ...
    it "allocates the latest active image for read-only volumes" do
      ...
      vm = create_vm
      described_class.allocate(vm, [{"size_gib" => 5, "use_bdev_ubi" => false, "boot" => true}, {"size_gib" => 0, "read_only" => true, "image" => "ai-model-test-model", "boot" => false, "use_bdev_ubi" => false}])
      expect(vm.vm_storage_volumes.first.boot_image_id).to eq(bi.id)
      expect(vm.vm_storage_volumes.last.boot_image_id).to eq(mi.id)
    end

    it "doesn't allocate a key encryption key for read-only volumes" do
      BootImage.create(vm_host_id: VmHost.first.id, name: "ai-model-test-model", version: "20240404", activated_at: Time.now, size_gib: 3)
      vm = create_vm
      described_class.allocate(vm, [{"size_gib" => 5, "use_bdev_ubi" => false, "boot" => true}, {"size_gib" => 0, "read_only" => true, "image" => "ai-model-test-model", "boot" => false, "use_bdev_ubi" => false}])
      expect(vm.storage_secrets.count).to eq(1)
      expect(vm.vm_storage_volumes.first.key_encryption_key_1_id).not_to be_nil
      expect(vm.vm_storage_volumes.last.key_encryption_key_1_id).to be_nil
    end
    ...
    it "allocates standard-gpu VM correctly on GEX44 host" do
      ...
      described_class.allocate(vm, [{"size_gib" => 85, "use_bdev_ubi" => false, "boot" => false},
        {"size_gib" => 95, "use_bdev_ubi" => false, "boot" => false}], gpu_count: 1, gpu_device: "27b0")
      vmh.reload
      vm.reload
  ...
  describe "slice selection" do
    ...
    it "allocates VMs in slice correctly on arm64 host" do
      ...
      vm = create_vm_from_size("standard-2", "arm64")
      described_class.allocate(vm, [{"size_gib" => 40, "use_bdev_ubi" => false, "boot" => false},
        {"size_gib" => 40, "use_bdev_ubi" => false, "boot" => false}])
      vmh.reload
      vm.reload
      ...
      described_class.allocate(vm_b1, [{"size_gib" => 20, "use_bdev_ubi" => false, "boot" => false},
        {"size_gib" => 20, "use_bdev_ubi" => false, "boot" => false}])
      vmh.reload
      vm_b1.reload
      ...
      described_class.allocate(vm_b2, [{"size_gib" => 20, "use_bdev_ubi" => false, "boot" => false},
        {"size_gib" => 20, "use_bdev_ubi" => false, "boot" => false}])
      vmh.reload
      vm_b2.reload
      slice_b.reload
